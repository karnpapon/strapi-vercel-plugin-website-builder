...
import { headers } from '../LogTableHeaders';
import { DeploymentList } from '../DeploymentList';
import { EmptyStateLayout } from '@strapi/design-system/EmptyStateLayout';
import { requestPluginEndpoint } from '../../../../utils/requestPluginEndpoint';
import ExclamationMarkCircle from '@strapi/icons/ExclamationMarkCircle';
import DeploymentLast from '../DeploymentLast';
import { Box } from '@strapi/design-system/Box';

const fetchDeployments = () => requestPluginEndpoint('deployments');

export const LogTable = () => {
	const {
		data: queryDeployments,
		error,
		isLoading,
	} = useQuery('build-vercel', () => fetchDeployments());
	const rowCount = 5;
	...
	if (isLoading)
		return (
			<Box padding={0} background="neutral100">
				<EmptyStateLayout icon={<Loader>loading data...</Loader>} content="loading data..." />
			</Box>
		);
		...
		return (
			<Box padding={0} background="neutral100">
				<EmptyStateLayout
					...
					content="Error occured during fetching deployments"
				/>
			</Box>
		);

	const { data } = queryDeployments || {};
	const { deployments } = data || {};
	const { deployments: allDeployments } = deployments || {};
	const [latest, ...restOftheList] = allDeployments;

	return (
		<>
			<DeploymentLast deployment={latest} />
			<Table rowCount={rowCount} colCount={colCount}>
				{!isLoading && allDeployments.length ? (
					<>
						<LogTableHeaders />
						<Tbody>
							{restOftheList.map((deploy) => (
								<DeploymentList key={deploy.uid} deployment={deploy} />
							))}
						</Tbody>
					</>
				) : (
					<Box padding={0} background="neutral100">
						<EmptyStateLayout icon={<Illo />} content="You don't have any deployment yet..." />
					</Box>
				)}
			</Table>
		</>
